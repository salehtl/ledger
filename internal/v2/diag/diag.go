...
// Accounting is the "zero drops without notice" report over a window.
type Accounting struct {
	From, To time.Time
	// InboundTotal counts ARRIVALS ONLY. Reprocessing is reported alongside in
	// Reprocess and is never folded in.
	InboundTotal int64
	Arrival      map[string]int64
	Reprocess    map[string]int64
	// Rejections are the aggregated protocol-level refusals. They are daily
	// counts, so the window endpoints are rounded to whole UTC days.
	Rejections map[string]int64
	// Unaccounted counts rows this build cannot classify: an event or an
	// outcome the database accepted but the constants in this package do not
	// name. It is the drift detector between the CHECK constraint and the Go
	// enum — a report that claims "zero drops" has to say when it does not
	// understand its own data rather than silently under-count.
	Unaccounted int64
}

// Accounting counts diagnostics by (event, outcome) over [from, to), plus the
// aggregated rejections for the UTC days that window touches.
func (d *Diag) Accounting(ctx context.Context, from, to time.Time) (Accounting, error) {
	if err := d.check(); err != nil {
	...
	rows, err := d.Pool.Query(ctx, `SELECT event, outcome, count(*) FROM parse_diagnostics
	  WHERE received_at >= $1 AND received_at < $2 GROUP BY event, outcome`, from, to)
	if err != nil {
		return Accounting{}, sanitize("accounting", err)
	}
	defer rows.Close()
	for rows.Next() {
		...
		case EventArrival:
			acc.InboundTotal += n
			acc.Arrival[outcome] += n
			if !slices.Contains(arrivalOutcomes, outcome) {
				acc.Unaccounted += n
			}
		case EventReprocess:
			acc.Reprocess[outcome] += n
			if !slices.Contains(reprocessOutcomes, outcome) {
				acc.Unaccounted += n
			}
		default:
			acc.Unaccounted += n
	...
	rrows, err := d.Pool.Query(ctx, `SELECT reason, coalesce(sum(count),0) FROM smtp_rejections
