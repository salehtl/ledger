...
// Every render goes through MotionProvider: ProgressBar's fill is an m.div,
// and an unwrapped m.* renders with no features loaded — animations (and the
// clip-path this file asserts on) would be silently inert.
...
describe("ProgressBar", () => {
  it("clamps to 0..100 and sets aria-valuenow", () => {
    const { getByRole } = wrap(<ProgressBar pct={1.4} />);
    const bar = getByRole("progressbar");
    expect(bar).toHaveAttribute("aria-valuenow", "100");
    // clip-path, not width: width is a layout property, and the fill carries
    // .dither-mask, whose 2px dot grid a scale would stretch into ellipses.
    expect(fillOf(bar).style.clipPath).toBe("inset(0 0% 0 0)");
  });

  it("clips proportionally to pct", () => {
    const { getByRole } = wrap(<ProgressBar pct={0.5} />);
    expect(fillOf(getByRole("progressbar")).style.clipPath).toBe("inset(0 50% 0 0)");
  });

  it("prints ink dots while inside pace", () => {
    const { getByRole } = wrap(<ProgressBar pct={0.3} pace={0.5} label="Needs" />);
  ...
  it("turns amber once past pace but still inside budget", () => {
    const { getByRole } = wrap(<ProgressBar pct={0.7} pace={0.5} label="Wants" />);
    const fill = fillOf(getByRole("progressbar"));
  ...
  it("turns red once past budget", () => {
    const { getByRole } = wrap(<ProgressBar pct={1.12} pace={0.5} label="Wants" />);
    const fill = fillOf(getByRole("progressbar"));
  ...
  it("stays dotted in every state — over is a colour change, not a texture change", () => {
    for (const pct of [0.3, 0.7, 1.4]) {
      const { getByRole, unmount } = wrap(<ProgressBar pct={pct} pace={0.5} />);
      const fill = fillOf(getByRole("progressbar"));
  ...
  it("never reads over-pace without a pace — an open-ended project has nothing to be ahead of", () => {
    const { getByRole } = wrap(<ProgressBar pct={0.9} label="Trip" />);
    const fill = fillOf(getByRole("progressbar"));
    expect(fill).toHaveAttribute("data-state", "under");
    expect(fill.style.background).toBe("var(--color-pace-under)");
  });
  ...
  it("still reads over-budget without a pace", () => {
    const { getByRole } = wrap(<ProgressBar pct={1.05} label="Trip" />);
  ...
  it("an explicit status overrides the geometric reading", () => {
    ...
    const { getByRole } = wrap(<ProgressBar pct={0.2} pace={0.5} status="over" label="Saving" />);
    const fill = fillOf(getByRole("progressbar"));
    expect(fill).toHaveAttribute("data-state", "over");
    expect(fill.style.background).toBe("var(--color-pace-over)");
  });
  ...
  it("an explicit status can pull a high pct back to under", () => {
    const { getByRole } = wrap(<ProgressBar pct={1.5} status="under" label="Projection" />);
    expect(fillOf(getByRole("progressbar"))).toHaveAttribute("data-state", "under");
  });

  it("draws a pace marker at the given fraction", () => {
    const { getByRole } = wrap(<ProgressBar pct={0.6} pace={0.5} />);
  ...
  it("onAccent never hardcodes white — the hero panel inverts between themes, so a literal bg-white fill is invisible in dark", () => {
    const { getByRole } = wrap(<ProgressBar pct={0.6} pace={0.5} onAccent />);
  ...
  it("onAccent carries state as texture, not ink — neither pace hue clears 3:1 on the hero ground in both themes", () => {
    const over = wrap(<ProgressBar pct={0.7} pace={0.5} onAccent />);
    ...
    wrap(<ProgressBar pct={1.2} pace={0.5} onAccent label="Total" />);
