...
| `src/screens/onboarding/DevSignInPanel.tsx` | **Development builds only, and provably absent from a production one.** The `dev:<subject>` sign-in that `ledgerd serve --dev-auth` accepts, so the app can be driven in a simulator where the operator's security-key-protected Apple ID cannot be presented. Loaded by a `require` inside `SignInScreen`'s `__DEV__` ternary, which Metro constant-folds away in production — so the module, its copy and `auth/devAuth.ts` are not in the emitted bundle. Do **not** convert that to a static import plus `{__DEV__ && …}`: that hides the control and ships the bypass. It establishes its session through the same reducer, exchange and invite gate as Apple. |
