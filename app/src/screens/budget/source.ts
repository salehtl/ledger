...
export interface BudgetSnapshot { homeCurrency: string | null; buckets: Record<BudgetBucket, bigint>; income: bigint; unassigned: bigint; confirmedTransactions: number; historyDays: number; warming: boolean; excluded: { missingHomeRate: number; unparsed: number; unresolvedDuplicates: number; sameDuplicates: number } }
...
export function sqlBudgetSource(db: SqlDriver, mapping: BudgetMapping = DEFAULT_BUDGET_MAPPING): BudgetSource { return { read(nowMs) {
  ...
  return { homeCurrency: readMeta(db)?.homeCurrency ?? null, buckets, income, unassigned, confirmedTransactions, historyDays, warming: historyDays < 14 && confirmedTransactions < 10, excluded: { missingHomeRate: Number(stats["missing"] ?? 0), unparsed: Number(stats["unparsed"] ?? 0), unresolvedDuplicates: Number(stats["unresolved"] ?? 0), sameDuplicates: Number(stats["same_dup"] ?? 0) } };
} }; }
