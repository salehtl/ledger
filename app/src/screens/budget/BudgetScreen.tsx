...
export function BudgetScreen({ source, nowMs, onCurrencies, onImport, onClose }: { source: BudgetSource; nowMs: number; onCurrencies: () => void; onImport: () => void; onClose: () => void }) {
  ...
  return <View style={{ flex: 1, padding: t.space.lg, gap: t.space.md, backgroundColor: t.colors.bg }}>
    ...
    {!data.usable ? (
      // `data.usable` mirrors `projectionIsUsable` (source.ts), exactly as
      // CurrenciesScreen's `!view.usable` branch does. A stale or half-written
      // projection must never be summed and shown as a real 50/30/20 — the
      // buckets below are already zeroed in that snapshot, so this branch is
      // what stops them from being presented as fact.
      <Text testID="budget-rebuilding" style={[t.type.body, { color: t.colors.textMuted }]}>
