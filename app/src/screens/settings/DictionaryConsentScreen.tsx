...
/**
 * The opt-in that shares one merchant→category pattern with the global
 * dictionary, and the disclosure the plan specifies word for word.
 *
 * Offered from the review queue immediately after a confirmation writes a rule
 * back, because that is the one moment the user has just decided what a
 * merchant is and the entry to share exists. It is off by default: nothing is
 * sent until the checkbox and the button are both pressed, and "Not now"
 * dismisses it leaving the user's own rule untouched.
 */
export function DictionaryConsentScreen({ submitter, entry, onDone, onDismiss }: { submitter: DictionarySubmitter; entry: { pattern: string; match: "exact" | "contains"; category: string }; onDone?(): void; onDismiss?(): void }) {
  ...
  return <View testID="dictionary-consent-screen" style={{ flex: 1, padding: t.space.lg, gap: t.space.lg, backgroundColor: t.colors.bg }}>
    ...
    <Pressable testID="dictionary-consent-dismiss" accessibilityRole="button" onPress={() => onDismiss?.()} style={{ minHeight: 44, justifyContent: "center" }}><Text style={[t.type.body, { color: t.colors.textMuted }]}>Not now</Text></Pressable>
